...
const Countdown = ({ params }: { params: { lang: Language } }) => {
  ...
  const {
    root: {
      misc: { day_abbrev, until_the_jam },
    },
  } = getDictionary(lang);
  const formatWithLeadingZero = (value: any) =>
  ...
  useEffect(() => {
    const targetDate = new Date("January 10, 2025 17:00:00 EST");
  ...
  return (
    <>
      <div className="flex items-center space-x-2 text-lg font-medium w-72">
        <div>
          <span className="text-2xl pr-2">{timeLeft.months}</span>
          <span>M</span>
        </div>
        <div>
        ...
        <div className="flex items-baseline">
          <span className="text-2xl w-10 text-left">
